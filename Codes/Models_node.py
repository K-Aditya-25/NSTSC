...
# Commenting out the following two classes as they are rarely identified as best models in practice.
...
#         self.beta4 = torch.nn.Parameter(torch.tensor(1.), requires_grad=True)
    ...
    def forward(self,x1, x2, x3):
        ...
        @return Output tensor.
        """
        #Make sure all the inputs are on the same device
        x1 = x1.to(device)
        x2 = x2.to(device)
        x3 = x3.to(device)
...
# # Commenting out the Preprocess function as it is not used in the current implementation.
