...
"""
@file Models_node.py
@brief Contains neural network models for temporal logic operations (conjunction, disjunction, etc.)

Implements a set of PyTorch modules for different temporal logic predicates for use in NSTSC.
"""
...
class TL_NN1(nn.Module):
    """
    @class TL_NN1
    @brief Neural network for conjunction of different predicates (AND operation).
    """
    def __init__(self, T):
        """
        @brief Initializes the TL_NN1 module.
    ...
    def forward(self,x1, x2, x3):
        ...
        # Ensure all input tensors are moved to the CUDA device
        x1 = x1.to(device)
        x2 = x2.to(device)
        x3 = x3.to(device)
        # print(f"x1 device: {x1.device}")
        # print(f"t1 type: {type(self.t1)}, t1 device: {self.t1.device}")
...
class TL_NN2(nn.Module):
    """
    @class TL_NN2
    @brief Neural network for disjunction of different predicates (OR operation).
    """
    def __init__(self, T):
        """
        @brief Initializes the TL_NN2 module.
        @param T Number of time steps.
        """
...
class TL_NN3(nn.Module):
    """
    @class TL_NN3
    @brief Neural network for always one predicate (Globally operator).
    """
    def __init__(self, T):
        """
        @brief Initializes the TL_NN3 module.
        @param T Number of time steps.
        """
    ...
    def forward(self,x1, x2, x3):
        """
        @brief Forward pass of the TL_NN3 module.
        @param x1 First input tensor.
        @param x2 Second input tensor.
        @param x3 Third input tensor.
        @return Output tensor.
...
class TL_NN4(nn.Module):
    """
    @class TL_NN4
    @brief Neural network for eventually one predicate (Finally operator).
    """
    def __init__(self, T):
        """
        @brief Initializes the TL_NN4 module.
        @param T Number of time steps.
        """
    ...
    def forward(self,x1, x2, x3):
        """
        @brief Forward pass of the TL_NN4 module.
        @param x1 First input tensor.
        @param x2 Second input tensor.
        @param x3 Third input tensor.
        @return Output tensor.
...
class TL_NN5(nn.Module):
    """
    @class TL_NN5
    @brief Neural network for always-eventually one predicate (Globally-Finally operator).
    """
    def __init__(self, T):
        """
        @brief Initializes the TL_NN5 module.
        @param T Number of time steps.
        """
    ...
    def forward(self,x1, x2, x3):
        """
        @brief Forward pass of the TL_NN5 module.
        @param x1 First input tensor.
        @param x2 Second input tensor.
        @param x3 Third input tensor.
        @return Output tensor.
        ...
        # self.activate1_1 = torch.max(torch.zeros(1).to(device), torch.min(torch.ones(1).to(device),self.weightbias1_1)).reshape([-1,self.T])
        self.activate1_1 = torch.max(torch.zeros(1, device = self.weightbias1_1.device), torch.min(torch.ones(1, device = self.weightbias1_1.device),self.weightbias1_1)).reshape([-1,self.T])         
...
class TL_NN6(nn.Module):
    """
    @class TL_NN6
    @brief Neural network for eventually-always one predicate (Finally-Globally operator).
    """
    def __init__(self, T):
        """
        @brief Initializes the TL_NN6 module.
        @param T Number of time steps.
        """
...
def Preprocess(x, T1, T2):
    ...
    @param T1 Start column
    @param T2 End column
    @return Sliced input
    """
    # print(f"x type: {type(x)}") #torch tensor
    # x = x.to(device)
    # print(f"x device: {x.device}")
    # xnew = torch.zeros([x.shape[0], T1, T2]).to(device)
    xnew = torch.zeros([x.shape[0], T1, T2], device= device)
    # print(f"x new type: {type(xnew)}") #torch tensor
    # print(f"xnew device: {xnew.device}")
    for i in range(T1):
        # xnew[:,i,:] = torch.cat((x[:, i:], torch.zeros([x.shape[0],i]).to(device)),1)
        xnew[:,i,:] = torch.cat((x[:, i:], torch.zeros([x.shape[0],i], device=device)),1)
    return xnew
